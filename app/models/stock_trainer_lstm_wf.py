...
def train_eval(tr_end, te_start, te_end):
    ...
    model = DualLSTMModel(len(feat)).to(device)
    ...
    return roc_auc_score(y[tem], tp), best

print("===== LSTM Walk-Forward =====")
res = []
for tr_end, te_s, te_e in FOLDS:
    out = train_eval(tr_end, te_s, te_e)
    if out is None:
        print(f"{te_s} ~ {te_e}  스킵"); continue
    fwd, val = out
    res.append(fwd)
    flag = "✅정방향" if fwd > 0.5 else "❌반전"
    print(f"{te_s} ~ {te_e}  test_AUC={fwd:.4f}  (val={val:.4f})  {flag}")

if res:
    print(f"\n평균 정방향 AUC: {np.mean(res):.4f}  (편차 {np.std(res):.4f})")